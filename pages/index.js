...
export default function Home() {
  ...
  const totalImagesCount = serviceAgents.reduce((total, product) => total + product.images.length, 0);
  const totalPrice = serviceAgents.reduce((total, product) => total + product.price, 0);
      ...
      <button onClick={() => signOut()}>Sign out</button> */}
            ...
            <div className="sm:flex sm:items-center sm:justify-between">
              <div className="text-center sm:text-left">
                <h1 className="text-3xl font-bold text-gray-900 sm:text-3xl">
                  Welcome Back, <span className="font-bold text-green-700">{session.user.name}</span>
                </h1>

                <p className="mt-1.5 text-md text-gray-500 max-w-md">
                  View the statistics about your business. Also manage and add ServiceAgents.
                </p>
              </div>

              <div className="flex flex-col gap-4 mt-4 sm:mt-0 sm:flex-row sm:items-center">
                <Link href={'/serviceAgents'}
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 lg:gap-8">
          <div className="flex items-center justify-center h-32 bg-gray-200 rounded-lg">
            <article
              className="flex items-end justify-between gap-4 rounded-lg max-md:flex-col"
            >
              <div>
          ...
          <div className="flex items-center justify-center h-32 bg-gray-200 rounded-lg">
            ...
            >
              <div>
                <p className="text-sm text-gray-500">ServiceAgents</p>

                <p className="text-2xl font-medium text-gray-900">{serviceAgents.length}</p>
              </div>
              <div className="inline-flex gap-2 p-1 text-green-600 bg-green-100 rounded">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                ...
                <span className="text-xs font-medium"> {serviceAgents.length} </span>
              </div>
            </article>
          </div>
          <div className="flex items-center justify-center h-32 bg-gray-200 rounded-lg">
          ...
          <div className="flex items-center justify-center h-32 bg-gray-200 rounded-lg">
            ...
            >
              <div>
                ...
                <p className="text-2xl font-medium text-gray-900">{services.length}</p>
              </div>
              <div className="inline-flex gap-2 p-1 text-green-600 bg-green-100 rounded">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                ...
                <span className="text-xs font-medium"> {services.length} </span>
              </div>
            </article>
    ...
    >
      <div className="max-w-xl lg:max-w-3xl">
        ...
        <p className="max-w-sm mt-4 leading-relaxed text-gray-500">
          This website is only accessible to admins only. Add new serviceAgents and manage database.
        </p>
